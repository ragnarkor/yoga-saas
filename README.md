...
- **超级管理员**：密码登录（Popup 弹窗，会员端「设置→后台管理」或教练版「超管密码登录」），进入**平台后台**；管某一馆内容时在平台后台点「进入教练版」，或在教练版顶栏切换馆。**无需微信绑定**
- **馆长 / 教练**：使用**微信绑定码**绑定后，进教练版**自动登录**，无需密码
- 所有密码统一为 `123456`（仅超管或备用登录使用）

### 微信绑定（馆长 / 教练）

1. **超管**密码登录（会员端设置→后台管理 Popup）→ 平台后台 → 选择馆「进入教练版」；或教练版「超管密码登录」Popup
