...
SRC_NAMES = ["coach_member_hero_preview.png", "calendar_index_bg_preview.png"]
OUT = os.path.join(
    ROOT, "miniprogram", "pages", "coach", "images", "coach_marketing_hero.jpg"
)
TARGET_W = 1500
TARGET_H = 680
# 会员页偏左 (-0.06)，营销页偏右裁切
...
def resolve_source() -> str:
    for d in ASSET_DIRS:
        for name in SRC_NAMES:
            ...
            if os.path.isfile(p):
                return p
    member_jpg = os.path.join(
        ROOT, "miniprogram", "pages", "coach", "images", "coach_member_hero.jpg"
    )
    if os.path.isfile(member_jpg):
        return member_jpg
    raise FileNotFoundError("marketing hero source not found")
...
def export_cover(src_path: str, out_path: str) -> None:
    ...
    print(f"saved {out_path} from {os.path.basename(src_path)}: {len(buf.getvalue()) // 1024} KB")
