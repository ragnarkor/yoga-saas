...
Page({
      ...
      success: async (res) => {
        switch (res.tapIndex) {
          case 0: {
            ...
            wx.navigateTo({
              url:
                "../record/admin_record_list?meetId=" +
                meetId +
                "&title=" +
                title,
            });
          ...
          case 1: {
            ...
            wx.navigateTo({
              url:
                "../export/admin_join_export?meetId=" +
                meetId +
                "&title=" +
                title,
            });
          ...
          case 0: {
            ...
            wx.navigateTo({
              url: pageHelper.fmtURLByPID(
                "/pages/meet/detail/meet_detail?id=" + meetId,
              ),
            });
            break;
          }
  ...
  _setSort: async function (meetId, sort, that) {
    ...
    try {
      await cloudHelper
        .callCloudSumbit("admin/meet_sort", params)
        .then((res) => {
  ...
  _del: async function (meetId, that) {
    ...
    let callback = async function () {
      try {
        ...
        await cloudHelper
          .callCloudSumbit("admin/meet_del", params, opts)
          .then((res) => {
  ...
  _setStatus: async function (meetId, status, that) {
    ...
    try {
      await cloudHelper
        .callCloudSumbit("admin/meet_status", params)
        .then((res) => {
          pageHelper.modifyListNode(
            meetId,
            that.data.dataList.list,
