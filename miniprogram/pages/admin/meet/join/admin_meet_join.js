...
Page({
  ...
  data: {
    ...
    sortMenus: [],
    sortItems: [],
    // [AI_END LINES=3 TIMESTAMP=2025-01-26 10:00:00]
  },
  ...
  bindCancelAllCmpt: async function () {
    try {
      ...
      await cloudHelper
        .callCloudSumbit("admin/meet_cancel_time_join", params, opt)
        .then((res) => {
          let callback = () => {
            let parent = pageHelper.getPrevPage(2);
            if (parent) {
              let daysSet = parent.data.daysSet;
              daysSet[this.data.parentDayIdx].times[
                this.data.parentTimeIdx
              ].stat = res.data;
              parent.setData({
                daysSet,
              });
  ...
  bindCheckinTap: async function (e) {
    ...
    let callback = async () => {
      ...
      try {
        await cloudHelper
          .callCloudSumbit("admin/join_checkin", params, opts)
          .then((res) => {
            let cb = () => {
              let sortIndex =
                this.selectComponent("#cmpt-comm-list").getSortIndex();
              if (sortIndex >= 10 && !this.data.search) {
                // 全部或者检索的结果
    ...
    else if (flag == 0) pageHelper.showConfirm("确认「取消签到」？", callback);
  },
  ...
  bindDelTap: async function (e) {
    let callback = async () => {
      ...
      try {
        await cloudHelper
          .callCloudSumbit("admin/join_del", params, opts)
          .then((res) => {
            let cb = () => {
              let dataList = this.data.dataList;
              dataList.list.splice(idx, 1);
  ...
  bindStatusTap: async function (e) {
    ...
    let callback = async () => {
      ...
      try {
        await cloudHelper
          .callCloudSumbit("admin/join_status", params, opts)
          .then((res) => {
            pageHelper.showSuccToast("操作成功", 1000);
            let sortIndex =
              this.selectComponent("#cmpt-comm-list").getSortIndex();

            if (sortIndex != -1 && sortIndex != 5 && !this.data.search) {
              // 全部或者检索的结果
              dataList.list.splice(idx, 1);
              dataList.total--;
            } else {
