...
class MeetService extends BaseService {
  ...
  async getDaysSet(meetId, startDay, endDay = null) {
    ...
    if (startDay && endDay && endDay == startDay) where.day = startDay;
    else if (startDay && endDay) where.day = ["between", startDay, endDay];
    else if (!startDay && endDay) where.day = ["<=", endDay];
    else if (startDay && !endDay) where.day = [">=", startDay];

    let orderBy = {
      day: "asc",
    };
    let list = await DayModel.getAllBig(
      where,
      "day,dayDesc,times",
      orderBy,
      1000,
  ...
  async join(userId, meetId, timeMark, forms, cardId, options = {}) {
    ...
    data.JOIN_START_TIME = timeUtil.time2Timestamp(
      daySet.day + " " + timeSet.start + ":00",
    );
  ...
  async checkMeetLimitSet(userId, meet, timeMark) {
    ...
    this._meetLog(
      meet,
      `#预约次数规则,预约日期=<${daySet.day}>`,
      `预约时段=[${timeSet.start}～${timeSet.end}]`,
    );
      ...
      for (let j in dayNode.times) {
        ...
        if (!timeNode.error) {
          try {
            ...
            if (ex.name == "AppError") timeNode.error = "预约结束";
            else throw ex;
          }
        }
  ...
  async detailForJoin(userId, meetId, timeMark) {
    ...
    let dayDesc = timeUtil.fmtDateCHN(
      this.getDaySetByTimeMark(meet, timeMark).day,
    );
  ...
  async getMeetList({
    ...
    let result = await MeetModel.getList(
      where,
      fields,
      orderBy,
      page,
  ...
  async cancelMyJoin(userId, joinId) {
    ...
    let meet = await this.getMeetOneDay(
      join.JOIN_MEET_ID,
      join.JOIN_MEET_DAY,
      whereMeet,
    );
    ...
    if (now > startTime) this.AppError("该预约已经开始，无法取消");
  ...
  async getMyJoinList(
    ...
    let fields =
      "JOIN_IS_CHECKIN,JOIN_REASON,JOIN_MEET_ID,JOIN_MEET_TITLE,JOIN_MEET_DAY,JOIN_MEET_TIME_START,JOIN_MEET_TIME_END,JOIN_STATUS,JOIN_ADD_TIME";
